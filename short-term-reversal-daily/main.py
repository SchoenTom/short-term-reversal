...
Adapted for daily frequency data.
"""
...
FILES = {
    'str': f'{DATA_DIR}/[usa]_[short_term_reversal]_[daily]_[ew].csv',
    'resmom': f'{DATA_DIR}/[usa]_[resff3_12_1]_[daily]_[ew].csv',
    'ff3': f'{DATA_DIR}/F-F_Research_Data_Factors_daily.csv',
    'ff5': f'{DATA_DIR}/F-F_Research_Data_5_Factors_2x3_daily.csv',
    'q5': f'{DATA_DIR}/q5_factors_daily_2024.csv',
    'gpr': f'{DATA_DIR}/data_gpr_daily_recent.xls',
}
...
# =============================================================================
def extract_zips():
    """Automatically extract any ZIP files in data directory."""
...
def load_strategy(filepath):
    """Load strategy returns from Global Factor Data (daily)."""
...
def load_ff_factors():
    ...
    def parse(path):
        ...
        # Daily format: YYYYMMDD (8 digits)
        df = df[df.iloc[:, 0].str.match(r'^\d{8}$', na=False)].copy()
...
def load_gpr(frequency='daily'):
    """Load Geopolitical Risk Index (Caldara & Iacoviello 2022)."""
    if not os.path.exists(FILES['gpr']):
        return None
    df = pd.read_excel(FILES['gpr'])

    # Handle daily GPR file format (DAY column in YYYYMMDD format)
    ...
    if frequency == 'daily' and 'GPRD' in df.columns:
        ...
        return df[gpr_col].astype(float)
...
def ols_regression(y, X, factor_names, model_name):
    ...
    return {
        'model': model_name,
        'alpha_ann': beta[0] * TRADING_DAYS_PER_YEAR,
        'alpha_t': t[0],
        'alpha_p': p[0],
        'r2': r2,
        'adj_r2': adj_r2,
...
def calc_performance_metrics(ret):
    """Calculate comprehensive performance metrics (daily data)."""
...
def gpr_regression(ret, gpr_data, frequency='daily'):
    """Regress returns on GPR changes."""
    df = pd.concat([ret.rename('ret'), gpr_data.rename('gpr')], axis=1).dropna()
    df['gpr_chg'] = df['gpr'].diff()
    df = df.dropna()

    # Minimum observations: 252 for daily (1 year), 30 for monthly
    min_obs = 252 if frequency == 'daily' else 30
    if len(df) < min_obs:
        return None
...
# =============================================================================
def main():
    """Execute complete analysis following project info sheet (DAILY DATA)."""

    print("=" * 80)
    print("SHORT-TERM REVERSAL STRATEGY ANALYSIS (DAILY DATA)")
    ...
    # Load data
    str_ret = load_strategy(FILES['str'])
    resmom_ret = load_strategy(FILES['resmom'])
    ff = load_ff_factors()
    q = load_q_factors()

    print(f"\nData: {len(str_ret)} days | {str_ret.index.min().date()} to {str_ret.index.max().date()}")
    ...
    # Minimum observations for daily: ~2 years of data
    MIN_OBS_DAILY = 504
    ...
    for period_name, str_sub, ff_sub, str_q_sub, q_sub in periods:
        ...
        for y, X, factors, name in models_sub:
            if len(y) > MIN_OBS_DAILY and len(X) > MIN_OBS_DAILY:
                common_idx = y.index.intersection(X.index)
                if len(common_idx) > MIN_OBS_DAILY:
                    r = ols_regression(y.loc[common_idx], X.loc[common_idx], factors, name)
                    sig = '***' if r['alpha_p'] < 0.01 else '**' if r['alpha_p'] < 0.05 else '*' if r[
                                                                                                        'alpha_p'] < 0.10 else ''
                    print(
                        f"  {name:<6} α={r['alpha_ann']:>7.2%} t={r['alpha_t']:>5.2f} p={r['alpha_p']:>6.4f} {sig:>4}  R²={r['r2']:>6.4f} Adj.R²={r['adj_r2']:>6.4f}")
    ...
    if gpr_daily is not None:
        gpr_common_daily = str_ff.index.intersection(gpr_daily.index)
        if len(gpr_common_daily) > 252:
            gpr_daily_res = gpr_regression(str_ff.loc[gpr_common_daily], gpr_daily.loc[gpr_common_daily], 'daily')
            if gpr_daily_res:
                sig = '***' if gpr_daily_res['p'] < 0.01 else '**' if gpr_daily_res['p'] < 0.05 else '*' if \
                gpr_daily_res['p'] < 0.10 else ''
                print(f"\nDaily GPR Regression (N={gpr_daily_res['n']}):")
                print(
                    f"  Coefficient: {gpr_daily_res['coef']:>10.6f}  t={gpr_daily_res['t']:>6.2f}  p={gpr_daily_res['p']:>7.4f} {sig}")

                # Correlation interpretation
                print(f"\nInterpretation:")
                if gpr_daily_res['p'] > 0.05:
                    print(f"  The coefficient ({gpr_daily_res['coef']:.6f}, p={gpr_daily_res['p']:.2f}) indicates that")
                    ...
                    direction = "negative" if gpr_daily_res['coef'] < 0 else "positive"
                    print(f"  The coefficient ({gpr_daily_res['coef']:.6f}, p={gpr_daily_res['p']:.4f}) indicates")
                    print(f"  a significant {direction} exposure to geopolitical risk.")
        ...
        print("\n  GPR data file not found. Skipping GPR analysis.")
    ...
    for w in np.linspace(0, 1, 101):
        ...
        sr = port.mean() / port.std() * np.sqrt(TRADING_DAYS_PER_YEAR) if port.std() > 0 else 0
        if sr > best_sr:
            best_w, best_sr = w, sr
    ...
    print(
        f"{'Residual Momentum only':<30} {rm_perf['ann_return']:>9.2%} {rm_perf['ann_vol']:>9.2%} {rm_perf['sharpe']:>10.3f}")
    print(
        f"{f'Combined ({best_w:.0%}/{1 - best_w:.0%})':<30} {combined_perf['ann_return']:>9.2%} {combined_perf['ann_vol']:>9.2%} {combined_perf['sharpe']:>10.3f}")

    improvement = (combined_perf['sharpe'] - str_perf['sharpe']) / abs(str_perf['sharpe']) * 100 if str_perf[
                                                                                                        'sharpe'] != 0 else 0
    ...
    if gpr_daily is not None and len(gpr_common_daily) > 252:
        gpr_port_common = port_opt.index.intersection(gpr_daily.index)
        if len(gpr_port_common) > 252:
            gpr_port = gpr_regression(port_opt.loc[gpr_port_common], gpr_daily.loc[gpr_port_common], 'daily')
            if gpr_port and gpr_daily_res:
                print(f"\nGPR Exposure Comparison:")
                print(f"{'-' * 80}")
                print(
                    f"  STR only:     Coef={gpr_daily_res['coef']:>10.6f}  t={gpr_daily_res['t']:>6.2f}  p={gpr_daily_res['p']:>7.4f}")
                sig_port = '***' if gpr_port['p'] < 0.01 else '**' if gpr_port['p'] < 0.05 else '*' if gpr_port[
                                                                                                           'p'] < 0.10 else ''
                print(
                    f"  Combined:     Coef={gpr_port['coef']:>10.6f}  t={gpr_port['t']:>6.2f}  p={gpr_port['p']:>7.4f} {sig_port}")
    ...
    ax1.set_title('Short-term Reversal Strategy: Wealth Growth (1926-2024) [Daily]', fontsize=14, fontweight='bold')
    ...
    ax2.axvline(x=pd.Timestamp('1990-01-01'), color='black', linestyle='--', alpha=0.7, lw=1.5,
                label='Jegadeesh (1990)')
    ax2.set_xlabel('Date', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Portfolio Value ($)', fontsize=12, fontweight='bold')
    ax2.set_title('Publication Effect: Pre vs Post Jegadeesh (1990) [Daily]', fontsize=14, fontweight='bold')
    ...
    ax3.plot(cum_combined.index, cum_combined.values, lw=2.5, color='#d62728', ls='--',
             label=f'Combined ({best_w:.0%}/{1 - best_w:.0%})')
    ax3.axhline(y=1, color='gray', linestyle=':', alpha=0.5)
    ax3.set_xlabel('Date', fontsize=12, fontweight='bold')
    ax3.set_ylabel('Portfolio Value ($)', fontsize=12, fontweight='bold')
    ax3.set_title('Portfolio Optimization: STR vs Combined Strategy [Daily]', fontsize=14, fontweight='bold')
    ...
    ax4.plot(cum_combined.index, cum_combined.values, lw=2.5, ls='--', color='#d62728',
             label=f'Combined ({best_w:.0%}/{1 - best_w:.0%})')
    ax4.axhline(y=1, color='gray', linestyle=':', alpha=0.5)
    ax4.set_xlabel('Date', fontsize=12, fontweight='bold')
    ax4.set_ylabel('Portfolio Value ($)', fontsize=12, fontweight='bold')
    ax4.set_title('Strategy Comparison: STR, Residual Momentum, Combined [Daily]', fontsize=14, fontweight='bold')
    ...
    # Figure 5: Rolling 252-day returns (1 year for daily data)
    fig5, ax5 = plt.subplots(figsize=(12, 6))
    rolling_ret = str_ff.rolling(TRADING_DAYS_PER_YEAR).apply(lambda x: (1 + x).prod() - 1) * 100
    ...
    ax5.set_ylabel('Rolling 252-Day Return (%)', fontsize=12, fontweight='bold')
    ax5.set_title('Short-term Reversal: Rolling Annual Returns [Daily]', fontsize=14, fontweight='bold')
    ...
    print("    - table5b_combined_factor_models.csv")
    print("    - table5c_combined_exposures.csv")
    print("  Figures:")
    print("    - figure1_str_wealth.png")
    print("    - figure2_str_publication_effect.png")
